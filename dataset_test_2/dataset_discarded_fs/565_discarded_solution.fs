...
export const smPart = defineFeature(function(context is Context, id is Id, definition is map)
    precondition { }
    {
        ...
        skPolyline(sketchRem2b, "polyRem2b", { "points" : [vector(-140.000000, 0.000000) * millimeter, vector(-145.338100, -40.000000) * millimeter, vector(-143.338100, -50.000000) * millimeter, vector(-197.549700, -50.000000) * millimeter, vector(-199.549700, -40.000000) * millimeter, vector(-140.000000, 180.000000) * millimeter, vector(8.000000, 180.000000) * millimeter, vector(8.000000, 0.000000) * millimeter, vector(-140.000000, 0.000000) * millimeter] });
        skSolve(sketchRem2b);
        ...
        sheetMetalFlange(context, id + "flange0_3", {
            "edges" : qClosestTo(qOwnedByBody(qBodyType(qCreatedBy(id + "smStart0", EntityType.BODY), BodyType.SOLID), EntityType.EDGE), vector(100.344800, 160.000000, 0.000000) * millimeter),
            "angleControlType" : SMFlangeAngleControlType.BEND_ANGLE,
            "bendAngle" : 90.000000 * degree,
            "limitType" : SMFlangeBoundingType.BLIND,
            "distance" : 10.000000 * millimeter,
        ...
        skPolyline(sketchRem3, "polyRem3", { "points" : [vector(0.000000, 0.000000) * millimeter, vector(-180.000000, 0.000000) * millimeter, vector(-220.000000, -172.413800) * millimeter, vector(-228.000000, -172.413800) * millimeter, vector(-228.000000, -26.896600) * millimeter, vector(-220.000000, -26.896600) * millimeter, vector(-180.000000, 180.000000) * millimeter, vector(0.000000, 180.000000) * millimeter, vector(0.000000, 0.000000) * millimeter] });
        skSolve(sketchRem3);
        sheetMetalTab(context, id + "smTab3", {
            "tabFaces" : qContainsPoint(qSketchRegion(id + "sketchRem3"), vector(100.344800, 160.000000, 5.000000) * millimeter),
            "booleanUnionScope" : qClosestTo(qCreatedBy(id + "flange0_3", EntityType.FACE), vector(100.344800, 160.000000, 5.000000) * millimeter),
            "booleanOffset" : 0.0 * millimeter
        });
    });