...
export const smPart = defineFeature(function(context is Context, id is Id, definition is map)
    precondition { }
    {
        ...
        sheetMetalFlange(context, id + "flange3_2", {
            "edges" : qClosestTo(qOwnedByBody(qBodyType(qCreatedBy(id + "smStart0", EntityType.BODY), BodyType.SOLID), EntityType.EDGE), vector(-81.722100, 141.000000, -3.494100) * millimeter),
            "angleControlType" : SMFlangeAngleControlType.BEND_ANGLE,
        ...
        skPolyline(sketchRem2, "polyRem2", { "points" : [vector(0.000000, 0.000000) * millimeter, vector(51.272600, -20.299600) * millimeter, vector(48.987322, -27.966239) * millimeter, vector(341.599522, -115.188639) * millimeter, vector(343.884800, -107.522000) * millimeter, vector(113.000000, 0.000000) * millimeter, vector(113.000000, 283.000000) * millimeter, vector(0.000000, 283.000000) * millimeter, vector(0.000000, 0.000000) * millimeter] });
        skSolve(sketchRem2);
        sheetMetalTab(context, id + "smTab2", {
            "tabFaces" : qContainsPoint(qSketchRegion(id + "sketchRem2"), vector(-83.150405, 141.000000, 1.297554) * millimeter),
            "booleanUnionScope" : qClosestTo(qCreatedBy(id + "flange3_2", EntityType.FACE), vector(-83.150405, 141.000000, 1.297554) * millimeter),
            "booleanOffset" : 0.0 * millimeter
        });
    });